...
export default function AttendanceView({ showToast, apiBase, currentUser }) {
  ...
  const fetchStudents = async () => {
    ...
    try {
      ...
      if (currentUser?.department) {
        queryParams.push(`dept=${encodeURIComponent(currentUser.department)}`);
      }
  ...
  const handleManualCheckIn = async (studentId, currentStatus) => {
    ...
    try {
      const res = await fetch(`${apiBase}/api/students/${endpoint}${deptQuery}`, {
        method: "POST",
    ...
    <div className="panel">
      <div className="panel-header">
        ...
        <div style={{ display: 'flex', gap: '10px' }}>
          ...
          <a href={`${apiBase}/api/students/export${currentUser?.department ? `?dept=${encodeURIComponent(currentUser.department)}` : ""}`} className="btn btn-primary" download>
